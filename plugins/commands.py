...
async def start(client, message):
    ...
    if len(message.command) == 2:
        ...
        if param.startswith("getfile-"):
            movie_name = param.replace("getfile-", "").replace("-", " ")
            await message.reply_text(f"🔍 Searching for: `{movie_name}`")

            # Simulate a message to trigger autofilter manually
            fake_msg = message
            fake_msg.text = movie_name

            try:
                # Call your autofilter search function manually
                from plugins.autofilter import auto_filter  # Adjust path if needed
                await auto_filter(client, fake_msg)
            except Exception as e:
                await message.reply_text("❌ Failed to search, please try again.")
                print("AutoFilter Error:", e)
            return

    # Default start message if no valid param
